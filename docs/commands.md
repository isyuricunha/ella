...
- **Quote of the week**: Triggered by a `workflow_dispatch` or `schedule` GitHub Action. Generates a short uplifting sentence, rewrites the quote line in the repo's `README.md`, and commits. Not a slash command - triggered by event type. On this repo, the `schedule` trigger is commented out in `.github/workflows/ella-mizuki.yml`; uncomment it to enable weekly auto-generation.
