...
Note: Only the repository owner can use slash commands."""
    ...
    def infer_commit_type(self, changed_files: list[str]) -> tuple[str, str | None]:
        ...
        if all(
            ...
            or path.endswith((".test.ts", ".test.tsx", ".test.mts", ".test.cts", ".test.js", ".test.jsx", ".test.mjs", ".test.cjs",
            ...
            or (path.endswith(".clj") and Path(path).name.endswith("_test.clj"))
            or (path.endswith(".cljs") and Path(path).name.endswith("_test.cljs"))
            or (path.endswith(".cljc") and Path(path).name.endswith("_test.cljc"))
            ...
            or path.endswith(".t")
            or (path.endswith(".v") and Path(path).name.endswith("_test.v"))
        ...
        if any(
            path.endswith(("package.json", "pnpm-lock.yaml", "package-lock.json", "yarn.lock", "bun.lockb", "bun.lock"))
